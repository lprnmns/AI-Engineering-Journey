...
| 10 | Kabul → identity → parse → normalize → chunk → embed → stage → verify → activate pipeline tablosu ve fingerprint formülü | `IngestionWorker`, SQLite registry, `PipelineConfig`, Qdrant verify/activate | PASS |
...
| 12 | Metadata sınıfları, filtre sırası ve privacy notları | tenant/ACL/document/active pre-filter, source re-check, trace hash/audit | PASS |
