...
| 16 | Run manifest, warm-up, random order, bootstrap CI, failure rate, latency budgets | `eval/reporting.py`, `eval/results/week2_report_v2/run_manifest.json`, manifest/raw artifact tests | PASS | Manifest source SHA `6e0c748`; run id, timestamp, chunk hash, model/config, host RAM, CI ve metric version alanları mevcut |
...
| 28 | `.env.example`, 5 ADR, resmi kaynaklar, aynı Git SHA ile final teslim | `.env.example`, `docs/adr/ADR-001..007`, README/API docs, `run_manifest.json` | PARTIAL | Artifact source SHA `6e0c748` manifestte; artifact paketleme commit’i farklı olacaktır, self-referential SHA üretilemez |
